...
import cn.edu.xmu.oomall.shop.service.dto.SimpleProductServiceDto;
...
public class ShopServiceTest {
    ...
    public void retrieveValidShops2(){
        List<SimpleShopDto> list = shopService.retrieveValidShops(Shop.SERVICE, "服务商", 1, 10).getList();
        assertThat(list.size()).isEqualTo(1);
        SimpleShopDto bo = list.get(0);
        assertThat(bo.getId()).isEqualTo(45L);
        assertThat(bo.getName()).isEqualTo("服务商");
    ...
    public void findShopById1(){
        ...
        ShopDto retShop =  shopService.findShopById(45L, user);

        assertThat(retShop.getId()).isEqualTo(Long.valueOf(45));
        assertThat(retShop.getName()).isEqualTo("服务商");
    ...
    public void findShopById2(){
        ...
        assertThrows(BusinessException.class, ()-> shopService.findShopById(100L, user));
    }
    ...
    public void updateShopStatus1(){
        ReturnObject ret = shopService.updateShopStatus(8L, Shop.OFFLINE, user);
        Assertions.assertThat(ret.getCode()).isEqualTo(ReturnNo.OK);
    }
    ...
    public void createProductService1(){
        ...
        Mockito.when(freightDao.findRegionById(10L)).thenReturn(new Region(){
            {
                setId(10L);
            }
        });
        SimpleProductServiceDto serviceDto = shopService.createProductService(45L, 5107L, 45L, 10L, LocalDateTime.now(), LocalDateTime.now(), null, null, user);
        ...
        assertThat(serviceDto.getShop().getId()).isEqualTo(45L);
    }

    @Test
    public void createProductService2(){
        assertThrows(BusinessException.class, ()-> shopService.createProductService(50L, 5107L, 50L, 10L, LocalDateTime.now(), LocalDateTime.now(), null, null, user));
    }

    @Test
    public void createProductService3(){
        assertThrows(BusinessException.class, ()-> shopService.createProductService(1L, 5107L, 1L, 10L, LocalDateTime.now(), LocalDateTime.now(), null, null, user));
    }

    @Test
    public void createProductService4(){
        Mockito.when(goodsDao.retriveProductById(80000L)).thenReturn(null);
        Mockito.when(freightDao.findRegionById(10000L)).thenReturn(null);
        assertThrows(BusinessException.class, ()-> shopService.createProductService(45L, 80000L, 45L, 10000L, LocalDateTime.now(), LocalDateTime.now(), null, null, user));
    }
    ...
    public void retrieveProductServiceByProductIdAndRegionId1(){
        PageDto<SimpleShopDto> shopDtoPageDto =  shopService.retrieveProductServiceByProductIdAndRegionId(45L, 5107L, 152L, user , 1, 10);
        SimpleShopDto shopDto = shopDtoPageDto.getList().get(0);
        assertThat(shopDto.getId()).isEqualTo(45L);
        assertThat(shopDto.getName()).isEqualTo("服务商");
        assertThat(shopDto.getType()).isEqualTo(Shop.SERVICE);
    }

    @Test
    public void retrieveProductServiceByProductIdAndRegionId2(){
        PageDto<SimpleShopDto> shopDtoPageDto =  shopService.retrieveProductServiceByProductIdAndRegionId(45L, 5107L, 1L, user , 1, 10);
